...
class KNN:
    ...
    def find_distance(self, x):
        ...
        """
        dist = np.zeros((x.shape[0], self.data.shape[0]))
        for i in range(x.shape[0]):
            for j in range(self.data.shape[0]):
                dist[i][j] = np.sum(np.abs(x[i] - self.data[j])**self.p)**(1/self.p)
        return dist
    ...
    def k_neighbours(self, x):
        ...
        neigh_dists = np.zeros((x.shape[0], self.k_neigh))
        idx_of_neigh = np.zeros((x.shape[0], self.k_neigh))
        for i in range(x.shape[0]):
            neigh_dists[i] = np.sort(dist[i])[:self.k_neigh]
            idx_of_neigh[i] = np.argsort(dist[i])[:self.k_neigh]
    ...
    def predict(self, x):
        ...
        for i in range(x.shape[0]):
            if self.weighted:
                pred[i] = np.argmax(np.bincount(self.target[idx[i].astype(np.int64)], weights=1/dist[i]))
            else:
                pred[i] = np.argmax(np.bincount(self.target[idx[i].astype(np.int64)]))
